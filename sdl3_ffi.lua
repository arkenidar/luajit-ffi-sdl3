...
function AudioSpecs:print()
    print(string.format(
        'spec parameters: \nfreq=%s, \nformat=%s, \nformat bits=%s, \nis float %s,\nendianess=%d, \nis signed %s, \nchannels=%s \nsilence=%s, \nsamples=%s bytes,\nsize=%s bytes',
        self.freq, self.format, bit.band(self.format, 0xff), tostring(bit.band(0x1000, self.format) > 0),
        bit.band(0x100, self.format), tostring(bit.band(0xF000, self.format) > 0), self.channels, self.silence,
...
function M.audio_buffer_type(spec)
    ...
    if isfloat > 0 then
        if bitsize == 32 then
            ...
            error("unknown float buffer type bits:" .. tostring(bitsize))
        end
    else
        if bitsize == 16 then
            typebuffer = "short"
        elseif bitsize == 32 then
            ...
            error("unknown buffer type bits:" .. tostring(bitsize))
        end
    end
...
-- Initialize global SDL functions/constants
local function InitializeGlobalMetatableAccess(SDL)
...
InitializeGlobalMetatableAccess(M) -- Initialize global SDL functions/constants
